...
class AplicacionGUI(tk.Tk):
  ...
  def crear_widgets(self):
      ...
      tk.Label(self, text="Seleccione el Modelo:").grid(row=0, column=0, sticky='e')
      tk.Radiobutton(self, text="Masa Puntual", variable=self.modelo_seleccionado,
                     value='masa_puntual').grid(row=0, column=1, sticky='w')
      tk.Radiobutton(self, text="4DOF", variable=self.modelo_seleccionado,
                     value='4dof').grid(row=1, column=1, sticky='w')

      # Etiquetas y campos de entrada
      tk.Label(self, text="Ángulo de Ataque/Lanzamiento (grados):").grid(row=2, column=0, sticky='e')
      self.entrada_angulo = tk.Entry(self)
      self.entrada_angulo.grid(row=2, column=1)

      tk.Label(self, text="Velocidad Inicial (m/s):").grid(row=3, column=0, sticky='e')
      self.entrada_velocidad = tk.Entry(self)
      self.entrada_velocidad.grid(row=3, column=1)

      tk.Label(self, text="Altura Inicial (m):").grid(row=4, column=0, sticky='e')
      self.entrada_altura = tk.Entry(self)
      self.entrada_altura.grid(row=4, column=1)

      tk.Label(self, text="Densidad del Aire (kg/m³):").grid(row=5, column=0, sticky='e')
      self.entrada_densidad = tk.Entry(self)
      self.entrada_densidad.grid(row=5, column=1)
      ...
      self.etiqueta_tiempo_vuelo.grid(row=7, column=0, columnspan=2)

      self.etiqueta_alcance_maximo = tk.Label(self, text="Alcance Máximo: N/A")
      self.etiqueta_alcance_maximo.grid(row=8, column=0, columnspan=2)

      self.etiqueta_altura_maxima = tk.Label(self, text="Altura Máxima: N/A")
      self.etiqueta_altura_maxima.grid(row=9, column=0, columnspan=2)

      # Botones
      tk.Button(self, text="Calcular", command=self.calcular).grid(row=6, column=0)
      tk.Button(self, text="Reiniciar", command=self.reiniciar).grid(row=6, column=1)
      tk.Button(self, text="Graficar Trayectoria", command=self.graficar_trayectoria).grid(row=10, column=0)
      tk.Button(self, text="Exportar a CSV", command=self.exportar_csv).grid(row=10, column=1)
  ...
  def calcular(self):
      try:
          ...
          self.calculadora.establecer_parametros(angulo, velocidad, altura, densidad)
